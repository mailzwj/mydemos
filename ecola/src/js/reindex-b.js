...
KISSY.use("sizzle",function(S){
    ...
    function changeFormAction(){
		...
		E.on(document, "mouseup", function(){
			if(dragable){
				dragable = false;
				var url = 'ajax/update.php?id=' + D.attr(log, 'id');
    ...
    function loadData(s){   //周startDate
        var weekdays = S.all(".week-main:eq(0)>.col");
        S.getScript("ajax/getdata.php?s=" + new Date(s.replace(/-/g, "/")).getTime(), function(){
