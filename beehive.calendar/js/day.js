Calendar.prototype.header=function(){
	...
	this.week = this.language.weekArray; 
	
	
	this.c_h_tb_th.innerHTML=this.date.format("yyyy - MM - dd ")+"  （"+this.week[this.date.getDay()]+"）  ";
	if(this.date.format("yyyyMMdd")==new Date().format("yyyyMMdd")){
		this.c_h_tb_th.innerHTML+=this.language.today;
	}
...
var GridList=function(date,language){
	...
	this.c_g_div.style.height=(getWinHeight()-69)+'px';
	...
	this.c_g_tr_b_td_slot.appendChild(new Slot_div_j(date));
	this.c_g_tr_b_td_slot.id=date.format("yyyyMMdd")+'_td';
	if(isToday(date)){
